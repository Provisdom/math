...
(deftest pseudoinverse-test
  (t/with-instrument `la/pseudoinverse
    (t/is-spec-check la/pseudoinverse {:num-tests 20}))
  (t/with-instrument :all
...
(deftest solve-test
  (t/with-instrument `la/solve
    (t/is-spec-check la/solve {:num-tests 20}))
  (t/with-instrument :all
    (t/is= [] (la/solve [[]] []))
...
(deftest lu-decomposition-test
  (t/with-instrument `la/lu-decomposition
    (t/is-spec-check la/lu-decomposition {:num-tests 15}))
  (t/with-instrument :all
...
(deftest cholesky-decomposition-test
  (t/with-instrument `la/cholesky-decomposition
    (t/is-spec-check la/cholesky-decomposition {:num-tests 15}))
  (t/with-instrument :all
...
(deftest rectangular-cholesky-decomposition-test
  (t/with-instrument `la/rectangular-cholesky-decomposition
    (t/is-spec-check la/rectangular-cholesky-decomposition {:num-tests 10}))
  (t/with-instrument :all
    ;; Empty matrix returns nil
    (t/is= nil (la/rectangular-cholesky-decomposition [[]] 1e-4))
    ;; Full rank 2x2
    ;; - verify reconstruction (values may differ from Apache due to different algorithm)
...
(deftest qr-decomposition-test
  (t/with-instrument `la/qr-decomposition
    (t/is-spec-check la/qr-decomposition {:num-tests 15}))
  (t/with-instrument :all
...
(deftest rank-revealing-qr-decomposition-test
  (t/with-instrument `la/rank-revealing-qr-decomposition
    (t/is-spec-check la/rank-revealing-qr-decomposition {:num-tests 10}))
  (t/with-instrument :all
    ;; Empty matrix
...
(deftest eigen-decomposition-test
  (t/with-instrument `la/eigen-decomposition
    (t/is-spec-check la/eigen-decomposition {:num-tests 10}))
  (t/with-instrument :all
    ...
    (let [m [[2.0 0.0] [0.0 3.0]]
      ...
      (t/is (some? result))
      ;; Eigenvalues can be in any order, so check as sets
      (t/is= #{2.0 3.0} (set eigenvalues)))))

;;;SVD
(deftest sv-decomposition-test
  (t/with-instrument `la/sv-decomposition
    (t/is-spec-check la/sv-decomposition {:num-tests 10}))
  (t/with-instrument :all
...
(deftest condition-number-test
  (t/with-instrument `la/condition-number
    (t/is-spec-check la/condition-number {:num-tests 15}))
  (t/with-instrument :all
    (t/is (Double/isNaN (la/condition-number [[]])))
    ;; Match Apache test values
...
(deftest least-squares-test
  (t/with-instrument `la/least-squares
    (t/is-spec-check la/least-squares {:num-tests 15}))
  (t/with-instrument :all
...
(deftest covariance-matrix->correlation-matrix-test
  (t/with-instrument `la/covariance-matrix->correlation-matrix
    (t/is-spec-check la/covariance-matrix->correlation-matrix {:num-tests 10}))
  (t/with-instrument :all
...
(deftest correlation-matrix->covariance-matrix-test
  (t/with-instrument `la/correlation-matrix->covariance-matrix
    (t/is-spec-check la/correlation-matrix->covariance-matrix {:num-tests 10}))
  (t/with-instrument :all
...
(deftest minor-test
  (t/with-instrument `la/minor
    (t/is-spec-check la/minor {:num-tests 10}))
  (t/with-instrument :all
...
(deftest cofactor-test
  (t/with-instrument `la/cofactor
    (t/is-spec-check la/cofactor {:num-tests 10}))
  (t/with-instrument :all
...
(deftest cofactor-matrix-test
  (t/with-instrument `la/cofactor-matrix
    (t/is-spec-check la/cofactor-matrix {:num-tests 10}))
  (t/with-instrument :all
...
(deftest adjugate-test
  (t/with-instrument `la/adjugate
    (t/is-spec-check la/adjugate {:num-tests 10}))
  (t/with-instrument :all
...
(deftest matrix-power-test
  (t/with-instrument `la/matrix-power
    (t/is-spec-check la/matrix-power {:num-tests 10}))
  (t/with-instrument :all
    ;; Empty matrix
...
(deftest matrix-exp-test
  (t/with-instrument `la/matrix-exp
    (t/is-spec-check la/matrix-exp {:num-tests 5}))
  (t/with-instrument :all
    ;; Empty matrix returns nil
