...
(deftest log-gamma-derivative-test                          ;same as digamma
  ...
  (is= -10.423754943278137 (mf/log-gamma-derivative 0.1))
  (is= -0.5772156677920679 (mf/log-gamma-derivative 1))
  (is= 0.4853359658127715 (mf/log-gamma-derivative 2.1))
  (is= -0.42375494327813756 (mf/log-gamma-derivative 1.1))
  (is= -1.2200235564290474 (mf/log-gamma-derivative 0.7))
  (is= 0.7031566378697297 (mf/digamma 2.5))
  (is= 1.1031566378697297 (mf/digamma -2.5))
...
(deftest gamma-derivative-test
  ...
  (is= -99.16647290191281 (mf/gamma-derivative 0.1))
  (is= -0.5772156677920679 (mf/gamma-derivative 1))
  (is= 0.507897219192069 (mf/gamma-derivative 2.1))
  (is= -0.4031395915225494 (mf/gamma-derivative 1.1))
  (is= -1.0428235898368983 (mf/gamma-derivative -2.5))
  (is= -1.583658083378364 (mf/gamma-derivative 0.7)))

(deftest trigamma-test
  (is= m/inf+ (mf/trigamma m/inf+))
  (is (m/nan? (mf/trigamma m/nan)))
  (is= 101.43329915079268 (mf/trigamma 0.1))
  (is= 0.606852869800954 (mf/trigamma 2.1))
  (is= 1.4332991507926893 (mf/trigamma 1.1))
  (is= 1.6449340668481562 (mf/trigamma 1))
  (is= m/inf+ (mf/trigamma 0.0))
  (is= 2.8340491566945474 (mf/trigamma 0.7))
  (is= 101.92253995947712 (mf/trigamma -0.1))
