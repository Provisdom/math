...
(deftest trim-number-as-string-test
  (with-instrument `format/trim-number-as-string
...
(deftest format-as-float-test
  (with-instrument `format/format-as-float
...
(deftest format-as-exponential-test
  (with-instrument `format/format-as-exponential
  ...
  (with-instrument (st/instrumentable-syms)
    ...
    (is= "-2.343200E+4"
      (format/format-as-exponential -23432 {::format/digits 7}))
...
(deftest format-number-test
  (with-instrument `format/format-number
...
(deftest unparse-shorthand-test
  (with-instrument `format/unparse-shorthand
  ...
  (with-instrument (st/instrumentable-syms)
    ...
    (is= "323.23432412M"
      (format/unparse-shorthand
        3.23234324123124E8 13 {::format/max-decimal-places 2}))
    (is= "320M"
    ...
    (is= "-$323M"
      (format/unparse-shorthand -323234324 3 {::format/money? true}))))

(deftest parse-shorthand-test
  (with-instrument `format/parse-shorthand
