...
(declare clatrix rows columns diagonal some-kv transpose eigen-decomposition mx*
         correlation-clatrix-by-squaring
         positive-semidefinite-clatrix-finite-by-squaring
         positive-definite-clatrix-finite-by-squaring
         symmetric-clatrix-by-averaging add
...
(s/def ::clatrix
  (s/with-gen
    clatrix?
    #(gen/fmap clatrix (s/gen ::mx/matrix))))

(defn empty-clatrix?
  "Returns true is an empty Clatrix."
  [x]
  (and (clatrix? x) (zero? (rows x))))
...
(s/def ::clatrix-finite
  (s/with-gen
    clatrix-finite?
    #(gen/fmap clatrix (s/gen ::mx/matrix-finite))))
...
(s/def ::row-clatrix
  (s/with-gen
    row-clatrix?
    #(gen/fmap clatrix (s/gen ::mx/row-matrix))))
...
(s/def ::column-clatrix
  (s/with-gen
    column-clatrix?
    #(gen/fmap clatrix (s/gen ::mx/column-matrix))))

(defn square-clatrix?
  "Returns true if a square Clatrix."
  [x]
  (and (clatrix? x) (clatrix/square? x)))

(s/fdef square-clatrix?
        :args (s/cat :x any?)
        :ret boolean?)

(s/def ::square-clatrix
  (s/with-gen
    square-clatrix?
    #(gen/fmap clatrix (s/gen ::mx/square-matrix))))

(s/def ::square-clatrix-finite
  (s/with-gen
    (s/and square-clatrix? clatrix-finite?)
...
(s/def ::diagonal-clatrix
  (s/with-gen
    diagonal-clatrix?
    #(gen/fmap clatrix (s/gen ::mx/diagonal-matrix))))

(defn upper-triangular-clatrix?
  "Returns true if an upper triangular matrix (square with the entries below the
  main diagonal all zero)."
  [x]
  (and (square-clatrix? x)
       (nil? (some-kv (fn [r c e]
                        (not (or (<= r c) (zero? e))))
                      x))))
...
(s/def ::upper-triangular-clatrix
  (s/with-gen
    upper-triangular-clatrix?
    #(gen/fmap clatrix (s/gen ::mx/upper-triangular-matrix))))

(defn lower-triangular-clatrix?
  "Returns true if a lower triangular matrix (square with the entries above the
  main diagonal all zero)."
  [x]
  (and (square-clatrix? x)
       (nil? (some-kv (fn [r c e]
...
(s/def ::lower-triangular-clatrix
  (s/with-gen
    lower-triangular-clatrix?
    #(gen/fmap clatrix (s/gen ::mx/lower-triangular-matrix))))

(defn symmetric-clatrix?
  "Returns true is a symmetric Clatrix."
  [x]
  (and (clatrix? x) (=== (transpose x) x)))
...
(s/def ::symmetric-clatrix
  (s/with-gen
    symmetric-clatrix?
    #(gen/fmap clatrix (s/gen ::mx/symmetric-matrix))))
...
(s/def ::positive-semidefinite-clatrix-finite
  (s/with-gen
    #(positive-semidefinite-clatrix-finite? % m/sgl-close)
    #(gen/fmap (fn [m]
                 (positive-semidefinite-clatrix-finite-by-squaring (clatrix m)))
...
(s/def ::positive-definite-clatrix-finite
  (s/with-gen
    #(positive-definite-clatrix-finite? % m/sgl-close)
    #(gen/fmap (fn [m]
                 (positive-definite-clatrix-finite-by-squaring (clatrix m)))
               (s/gen ::mx/square-matrix-finite))))

(defn correlation-clatrix?
  "Returns true if a finite positive definite Clatrix with a unit diagonal (all
...
(s/def ::correlation-clatrix
  (s/with-gen
    #(correlation-clatrix? % m/sgl-close)
    #(gen/fmap (fn [m]
                 (correlation-clatrix-by-squaring (clatrix m)))
               (s/gen ::mx/square-matrix-finite))))
...
(defn positive-semidefinite-clatrix-finite-by-squaring
  "Returns a finite positive semidefinite Clatrix by first squaring
...
(defn positive-definite-clatrix-finite-by-squaring
  "Returns a finite positive definite Clatrix squaring it. Will tweak original
  matrix if necessary to ensure positive definite."
  [square-clatrix-m-finite]
  (let [size (rows square-clatrix-m-finite)]
    (if (zero? size)
      square-clatrix-m-finite
...
(defn correlation-clatrix-by-squaring
  "Returns a Correlation Clatrix by first squaring `square-clatrix-m`."
  [square-clatrix-m-finite]
  (if (zero? (rows square-clatrix-m-finite))
    ...
    (let [new-m (covariance-clatrix->correlation-clatrix
      ...
      (when new-m
        (loop [i 0]
          (when (< i 100)
            (let [lower-m (mx* new-m (clatrix
                                       (mx/diagonal-matrix
                                         (repeat size (m/one- (* 0.01 i))))))
                  lower-m (symmetric-clatrix-by-averaging lower-m)
                  lower-m (assoc-diagonal lower-m (repeat size 1.0))]
              (if (correlation-clatrix? lower-m m/sgl-close)
                lower-m
                (recur (inc i))))))))))

(s/fdef correlation-clatrix-by-squaring
        :args (s/cat :square-clatrix-m-finite ::square-clatrix-finite)
        :ret (s/nilable ::correlation-clatrix))

(defn rnd-positive-definite-clatrix-finite!
  "Returns a finite positive definite Clatrix with a random spectrum. The
  orthogonal matrices are generated by using 2 × `size` composed Householder
  reflections.
  Alternative #1: Sample from the Inverse-Wishart Distribution.
  Alternative #2: Use [[positive-definite-clatrix-by-squaring]] with a random
    square matrix."
  [size]
  (if (zero? size)
    (clatrix [[]])
...
(defn rnd-correlation-clatrix!
  "Returns a Correlation Clatrix from a covariance matrix with a random
  ...
  Alternative #2: Use [[correlation-clatrix-by-squaring]] with a random square
    matrix."
  [size]
  (if (zero? size)
    (clatrix [[]])
    (covariance-clatrix->correlation-clatrix
      (rnd-positive-definite-clatrix-finite! size))))

(s/fdef rnd-correlation-clatrix!
        :args (s/cat :size ::vector/size)
        :ret ::correlation-clatrix)

;;;MATRIX INFO
(defn rows
  "Returns the number of rows of a Clatrix."
  [clatrix-m]
  (clatrix/nrows clatrix-m))
...
(defn get-entry
  ...
  [clatrix-m row column]
  (try (clatrix/get clatrix-m row column)
       (catch Exception _ m/nan)))

(s/fdef get-entry
        :args (s/and (s/cat :clatrix-m ::clatrix
                            :row ::mx/row
                            :column ::mx/column)
                     (fn [{:keys [clatrix-m row column]}]
                       (and (< row (rows clatrix-m))
...
(defn get-row
  ...
  [clatrix-m row]
  (try (vec (clatrix/slice-row clatrix-m row))
       (catch Exception _ nil)))

(s/fdef get-row
        :args (s/and (s/cat :clatrix-m ::clatrix :row ::mx/row)
                     (fn [{:keys [clatrix-m row]}]
                       (< row (rows clatrix-m))))
...
(defn get-column
  ...
  [clatrix-m column]
  (try (vec (clatrix/slice-column clatrix-m column))
       (catch Exception _ nil)))

(s/fdef get-column
        :args (s/and (s/cat :clatrix-m ::clatrix :column ::mx/column)
                     (fn [{:keys [clatrix-m column]}]
                       (< column (columns clatrix-m))))
        :ret (s/nilable ::vector/vector))
...
(defn some-kv
  ...
  number in Clatrix, else nil. Options: `::mx/by-row?` (default: true)."
  ([pred clatrix-m] (some-kv pred clatrix-m {::mx/by-row? true}))
  ([pred clatrix-m {:keys [::mx/by-row?] :or {by-row? true}}]
   (let [mt (if by-row?
              clatrix-m
              (transpose clatrix-m))
         rows (rows mt)]
...
(s/fdef correlation-clatrix->covariance-clatrix
        :args (s/cat :correlation-clatrix-m-finite ::correlation-clatrix
...
(defn covariance-clatrix->correlation-clatrix
  ...
  (if (zero? (rows covariance-clatrix-m-finite))
    covariance-clatrix-m-finite
    (let [inv-sqrt (clatrix
                     (mx/diagonal-matrix
                       (map #(m/pow % -0.5)
                            (diagonal covariance-clatrix-m-finite))))
      ...
      (when (correlation-clatrix? corr m/sgl-close) corr))))
...
(defn ===
  ...
  ([clatrix-m1 clatrix-m2]
   (tensor/=== (clatrix->matrix clatrix-m1) (clatrix->matrix clatrix-m2)))
  ([clatrix-m1 clatrix-m2 & clatrix-ms]
   (apply tensor/===
          (clatrix->matrix clatrix-m1)
          (clatrix->matrix clatrix-m2)
          (map clatrix->matrix clatrix-ms))))
...
(defn mx*
  ...
  ([clatrix-m1 clatrix-m2 & clatrix-ms]
   (when-let [clatrix-m3 (mx* clatrix-m1 clatrix-m2)]
     (apply mx* clatrix-m3 clatrix-ms))))
  ...
  ([clatrix-m1 clatrix-m2]
   (if (and (zero? (rows clatrix-m1)) (zero? (rows clatrix-m2)))
     ...
     (try (clatrix/+ clatrix-m1 clatrix-m2)
          (catch Exception _ nil))))
  ([clatrix-m1 clatrix-m2 & clatrix-ms]
   (when-let [clatrix-m3 (add clatrix-m1 clatrix-m2)]
     (apply add clatrix-m3 clatrix-ms))))
  ...
  ([clatrix-m1 clatrix-m2]
   (if (and (zero? (rows clatrix-m1)) (zero? (rows clatrix-m2)))
     ...
     (try (clatrix/- clatrix-m1 clatrix-m2)
          (catch Exception _ nil))))
  ([clatrix-m1 clatrix-m2 & clatrix-ms]
   (when-let [clatrix-m3 (subtract clatrix-m1 clatrix-m2)]
     (apply subtract clatrix-m3 clatrix-ms))))
...
(defn inverse
  "Computes the inverse of a number, vector, or matrix. This is done via
  Gaussian elimination. It can be numerically very unstable if the matrix is
  nearly singular. Positivity and symmetry hints are used to cause the solve to
  use optimized LAPACK routines."
  [square-clatrix-m]
  (if (empty-clatrix? square-clatrix-m)
    (clatrix [[]])
    (try (clatrix/i
           (clatrix/maybe-positive
             (clatrix/maybe-symmetric square-clatrix-m)))
         (catch Exception _ nil))))
...
(defn eigen-decomposition
  "Returns map with a vector of the real parts of eigenvalues and a Clatrix of
  eigenvectors. A matrix can be decomposed as A = Q × L × Q^-1, where L is the
  diagonal matrix of `eigenvalues`, 'Q' is the `eigenvalues` matrix, and Q^-1 is
  the inverse of 'Q'."
  [square-clatrix-m]
  (if (empty-clatrix? square-clatrix-m)
    {::eigenvalues  []
...
(defn upper-cholesky-decomposition
  "Computes the Cholesky decomposition of a Clatrix. This is the Cholesky square
  root of a Clatrix, 'U' such that `positive-definite-clatrix-m` = UT × U. Note
  that `positive-definite-clatrix-m` must be positive (semi) definite for this
  to exist, but [[upper-cholesky-decomposition]] requires strict positivity."
...
(defn sv-decomposition
  "Calculates the compact Singular Value Decomposition of a Clatrix. The
  Singular Value Decomposition of `clatrix-m-finite` is a set of three matrices:
  `svd-left`, `singular-values`, and `svd-right` such that:
  `clatrix-m-finite` = `svd-left` × `singular-values` × `svd-right`.
  Let `clatrix-m-finite` be a m × n matrix, then `svd-left` is a m × p
  orthogonal matrix of the left singular vectors, `singular-values` is a p × p
  diagonal matrix of singular values with positive or nil elements, and are
  ordered from largest to smallest. `svd-right` is a p × n orthogonal matrix of
  the right singular vectors where p = min(m,n). Note that:
  Identity Matrix = (transpose `svd-left`) × `svd-left` = `svd-right` ×
    (transpose `svd-right`).
  Returns a map containing:
...
(defn condition
  "The `singular-values-clatrix` is the diagonal matrix of `::singular-values`
  from [[sv-decomposition]]. Returns the norm2 condition number, which is the
  maximum element value from the `singular-values-clatrix` divided by the
...
(defn qr-decomposition
  "Computes the QR decomposition of a Clatrix. Returns a map containing
  Clatrices `::Q` and `::R`:
