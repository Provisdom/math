...
(defn row-matrix?
  ...
  [x] (and (vector? x) (m/one? (count x)) (vector? (first x)) (every? number? (first x))))
...
(defn column-matrix?
  ...
  [x] (or (empty-matrix? x) (and (vector? x) (every? #(and (vector? %) (= (count %) 1) (every? number? %)) x))))
...
(s/def ::zero-matrix (s/with-gen (s/coll-of (s/coll-of zero? :kind vector? :into []) :kind vector? :into [])
...
(defn upper-triangular-matrix?
  ...
  [x] (and (square-matrix? x) (nil? (some-kv (fn [i j e] (not (or (<= i j) (zero? e)))) x))))
...
(s/def ::symmetric-matrix
  (s/with-gen symmetric-matrix?
              #(gen/bind (gen/large-integer* {:min 0 :max mdl})
                         (fn [i] (gen/fmap deserialize-symmetric-matrix
                                           (gen/vector (s/gen ::number)
                                                       (ecount-of-symmetric-or-triangular-matrix i)))))))
...
(defn constant-matrix
  ...
  ([rows columns number] (if (zero? rows) [[]] (vec (repeat rows (vec (repeat columns number)))))))
...
(defn diagonal-matrix
  ...
  ([diagonal-numbers]
   (let [d (vec diagonal-numbers)
         c (count d)]
     (compute-matrix c c (fn [r c] (if (= r c) (get d r 0.0) 0.0)))))
...
(s/fdef toeplitz-matrix
        :args (s/with-gen (s/and (s/cat :first-row ::vector :first-column ::vector)
                                 #(= (first (:first-row %)) (first (:first-column %))))
                          #(gen/fmap
                             (fn [[fr fc]] (let [ffr (first fr)
                                                 fc (if ffr (assoc fc 0 ffr) fc)]
                                             [fr fc]))
                             (gen/tuple (s/gen ::vector) (s/gen ::vector))))
        :ret ::matrix)
...
(defn diagonal
  ...
  ([m]
   (if (empty-matrix? m)
     []
     (reduce (fn [tot e] (conj tot (get-in m [e e]))) [] (range (min (rows m) (columns m))))))
  ([m k]
   (if (empty-matrix? m)
     []
     (let [r (if (neg? k) (- k) 0)
       ...
       (if (pos? end)
         (vec (for [i (range start end)] (get-in m [(+ i r) (+ i c)])))
         [])))))
...
(defn filter-by-row
  ...
  [pred m] (let [new-m (filter pred m)] (if (empty? new-m) [[]] (vec new-m))))

(s/fdef filter-by-row
        :args (s/cat :pred (s/fspec :args (s/cat :row-vector ::vector) :ret boolean?) :m ::matrix)
        :ret ::matrix)

(defn filter-by-column
  "Returns a matrix.
  'pred' takes a column-vector."
  [pred m] (let [new-m (filter pred (transpose m))] (if (empty? new-m) [[]] (transpose (vec new-m)))))

(s/fdef filter-by-column
        :args (s/cat :pred (s/fspec :args (s/cat :column-vector ::vector) :ret boolean?) :m ::matrix)
        :ret ::matrix)
...
(defn filter-symmetric-matrix
  ...
  [pred symmetric-m]
  (let [keep-set (reduce-kv #(if (pred %3) (conj % %2) %) #{} symmetric-m)]
    (get-slices-as-matrix symmetric-m {::row-indices keep-set, ::column-indices keep-set})))

(s/fdef filter-symmetric-matrix
        :args (s/cat :pred (s/fspec :args (s/cat :row-vector ::vector) :ret boolean?)
...
(defn ereduce-kv
  ...
  ([f init m]
   (let [nr (rows m)]
     (loop [r 0, val init, s m]
       (let [g (fn [result column number] (f result r column number))]
         (if (>= r nr)
           val
           (recur (inc r) (reduce-kv g val (first s)) (rest s)))))))
  ([f init m1 m2]
   (let [nr (min (rows m1) (rows m2))]
     (loop [r 0, val init, s1 m1, s2 m2]
       (let [g (fn [result column number1 number2] (f result r column number1 number2))]
         (if (>= r nr)
           val
           (recur (inc r)
  ...
  ([f init m1 m2 m3]
   (let [nr (min (rows m1) (rows m2) (rows m3))]
     (loop [r 0, val init, s1 m1, s2 m2, s3 m3]
       (let [g (fn [result column number1 number2 number3] (f result r column number1 number2 number3))]
         (if (>= r nr)
           val
           (recur (inc r)
...
(s/fdef matrix->sparse
        :args (s/cat :m ::matrix
                     :pred (s/? (s/fspec :args (s/cat :number ::number) :ret boolean?)))
        :ret ::sparse-matrix)
...
(s/fdef assoc-diagonal
        :args (s/cat :m ::matrix :numbers ::numbers)
        :ret (s/nilable ::matrix))
...
(defn update-row
  "Updates a `row` of matrix `m`, using `f`,
  which is a function of the `column` and `number` and returns a number."
  [m row f] (when (< row (rows m)) (update m row (fn [row-vector] (vec (map-indexed f row-vector))))))
...
(defn update-column
  ...
  (when (< column (columns m))
    (vec (map-indexed (fn [row row-vector] (update row-vector column #(f row %))) m))))
...
(defn update-diagonal
  ...
  [m f]
  (vec (map-indexed (fn [row row-vector] (if (< row (columns m))
...
(defn concat-rows
  ...
  ([m & ms]
   (let [c (columns m)
         cs (map columns ms)]
     (when (every? #(= c %) cs)
       (if (zero? c) [[]] (vec (apply concat m ms)))))))
  ...
  (let [tr (+ (rows submatrix) row-start)
    ...
    (vec (for [r (range (min row-start 0) (max tr nr))]
           (vec (for [c (range (min column-start 0) (max tc nc))]
                  (cond (and (>= r row-start) (< r tr) (>= c column-start) (< c tc))
                        (get-in submatrix [(- r row-start) (- c column-start)])
                        (and (m/non-? r) (< r nr) (m/non-? c) (< c nc)) (get-in m [r c])
                        :else 0.0)))))))
...
(defn mx*
  ...
  ([m1 m2]
   (when (= (columns m1) (rows m2))
     (if (empty-matrix? m1)
       [[]]
       (mapv (fn [a] (mapv (fn [b] (apply + (map (fn [i j] (* (double i) j)) a b))) (transpose m2))) m1))))
  ([m1 m2 & ms] (when-let [m3 (mx* m1 m2)] (apply mx* m3 ms))))
...
(defn kronecker-product
  ...
  ([m1 m2]
   (if (or (empty-matrix? m1) (empty-matrix? m2))
     [[]]
     (vec (apply concat (mapv (fn [row] (apply concat-columns (mapv (fn [e] (tensor/multiply e m2)) row))) m1)))))
...
;;;MATRIX ROUNDING
(defn- roughly-zero-fn [accu]
  (fn [v] (every? (fn [number] (m/roughly? number 0.0 (double accu))) v)))

(defn round-roughly-zero-rows
  "Returns a matrix after rounding any roughly-zero rows."
  [m accu]
  (mapv (fn [row] (if ((roughly-zero-fn accu) row)
...
(defn round-roughly-zero-columns
  ...
  [m accu] (transpose (round-roughly-zero-rows (transpose m) accu)))
