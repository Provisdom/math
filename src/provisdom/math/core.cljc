...
(s/def ::prob
  (s/spec prob?
    :gen #(s/gen (s/or :d (s/double-in :min 0.0 :max 1.0 :NaN? false)
                   :i #{0 1}))))

(s/def ::nan-or-prob
  (s/spec #(or (nan? %) (prob? %))
    :gen #(s/gen (s/or :d (s/double-in :min 0.0 :max 1.0)
                   :i #{0 1}))))
...
(s/def ::corr
  (s/spec corr?
    :gen #(s/gen (s/or :d (s/double-in :min -1.0 :max 1.0 :NaN? false)
                   :i #{0 -1 1}))))

(s/def ::nan-or-corr
  (s/spec #(or (nan? %) (corr? %))
    :gen #(s/gen (s/or :d (s/double-in :min -1.0 :max 1.0)
                   :i #{0 -1 1}))))
